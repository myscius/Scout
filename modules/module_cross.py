...
#         return torch.matmul(x,y.t()).squeeze(1)/(x_m * y_m)

#     def forward(self, visual, sequence):
#         visual_out = torch.zeros(visual.shape[0],1,visual.shape[-1])
#         batch = visual.shape[0]
#         if visual.shape[0] != sequence.shape[0]:
#             batch = np.min((visual.shape[0],sequence.shape[0]))
#         #print(batch)
#         for i in range(batch):
#             #print('curent i:',i, 'sequence_range:',sequence.shape[0],'visual_range:',visual.shape[0])
#             scores = self.scores(visual[i],sequence[i])
            
#             most_sim = torch.argmax(scores)
#             visual_out[i] = visual[i,most_sim,:]
