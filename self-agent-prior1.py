...
import sys
# 配置日志系统
log_tag = 'LVBCaption_prior1'  # 可以根据需要修改标签
Nf = 20
...
class SelfAgent(BaseAgent):
    ...
    def get_answer(self, video_path, query, sorted_frame_idx=None, subtitle_path=None, subtitle_mode=0, distribution_mode=0):
        ...
        if cache_key in self.dis_cache and distribution_mode!=0:
            ...
            print(f"Using cached distribution: {cache_key} -> ")
        else:
        ...
        if isinstance(distribution, dict):
            distribution['distribution'] = [0.1]*10
        logging.info(f"Question type and distribution used for sampling: {self.qstype}, {distribution}")
...
if __name__ == "__main__":
    ...
    # tag log info
    log_info = f"Prior 1 Log for frame-{Nf} with gemini on {bench_name}, subtitle_mode={subtitle_mode}, distribution_mode={distribution_mode}, cache_path={cache_path}"
