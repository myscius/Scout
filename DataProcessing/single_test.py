...
def agent_process(single_video_anno):
    for kkk, anno in tqdm(enumerate(single_video_anno), desc="processing items"):
        ...
        if len(answer_set) < 3:
            ...
            if anno['final_answer'] == chr(ord('A') + anno['correct_choice']):
                logger.info(f"first round, correct! {anno['correct_choice']}")
            else:
                logger.info(f"first round, wrong with final_answer: {anno['final_answer']}!")
            continue
        ...
        logger.info(answer_set)
        if len(answer_set) == 1:
            answer_set = next(iter(answer_set))
            answer_set = str(answer_set)
            if answer_set == chr(ord('A') + anno['correct_choice']):
                logger.info(f"second round, correct! {anno['correct_choice']}")
            else:
                logger.info(f"second round, wrong with final_answer: {answer_set}!")
            anno['final_answer'] = str(answer_set)
            continue

        logger.info(f"answer_dict: {answer_dict}, {chr(ord('A') + anno['correct_choice'])}")
        ...
        if final_answer == chr(ord('A') + anno['correct_choice']):  
            logger.info(f"third round, correct! {anno['correct_choice']}")
        else:
            logger.info(f"third round, wrong with final_answer: {anno['final_answer']}!")
...
if __name__ == "__main__":
    ...
    all_anno_path = './lvbench_subset_200.json'
    ...
    agent_process(single_video_anno)
