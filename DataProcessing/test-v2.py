...
import sys
# 配置日志系统
log_tag = 'VMME-v2_gemini'  # 可以根据需要修改标签
Nf = 20
...
class SelfAgent(BaseAgent):
    def __init__(self, model_name='Qwen/Qwen2.5-VL-7B-Instruct'):
        ...
        self.model_name = "gemini-3.1-flash-lite" # Qwen/Qwen3.5-9B
    ...
    def save_cache(self, cache_path="./tmp_distribution_cache.json"):
        if cache_path is None:
            return
...
def evaluate_dataset(agent, json_path, video_root=None, bench_name='lvb', subtitle_root=None, num_samples=None, subtitle_mode=0, distribution_mode=0):
    ...
    for item in tqdm(data, desc="Evaluating"):
        if bench_name=="LongVideoBench":
            ...
            candidates = item['options']
            correct_choice_idx = ord(item['answer'])-ord('A')  # Assuming answer is like "A", "B", etc.
            subtitle_rel_path = f"{video_id}.jsonl"
...
if __name__ == "__main__":
    ...
    video_root = "/data1/yangyan/benchmark/Video-MME-v2/videos"
    subtitle_root = "/data1/yangyan/benchmark/Video-MME-v2/subtitle"
    # json_path = "./lvbench_subset_50.json"
    json_path = "/data1/yangyan/workdir/vmme_v2_data.json"
    bench_name = 'Video-MME-v2'
    
    # tag log info
    log_info = f"Log for frame-{Nf} with gemini-3.1-flash-lite on {bench_name}, subtitle_mode={subtitle_mode}, distribution_mode={distribution_mode}, cache_path={cache_path}"
