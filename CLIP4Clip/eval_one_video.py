...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--eval_method',type=str, default =None, help='choose my eval method')
...
def dataloader_msrvtt_train(args, tokenizer):
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(msrvtt_dataset)  #一个需要看看的Sampler
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):  #训练主要过程
    global logger
    ...
    #writer = SummaryWriter('visualize')
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                ...
                start_time = time.time()
            #writer.add_scalar('loss', loss, global_step)  #在这里添加了损失图
    total_loss = total_loss / len(train_dataloader)
    return total_loss, global_step

def _run_on_single_gpu(model, batch_list_t, batch_list_v, batch_sequence_output_list, batch_visual_output_list):
    args = get_args()
    eval = args.eval_method
    sim_matrix = []
    for idx1, b1 in enumerate(batch_list_t):
        input_mask, segment_ids, *_tmp = b1
        sequence_output = batch_sequence_output_list[idx1]  #####在计算相似度矩阵时，先将每一个batch的seq_ouput取出
        each_row = []
        for idx2, b2 in enumerate(batch_list_v):
            video_mask, *_tmp = b2
            visual_output = batch_visual_output_list[idx2]   ######再将每一个batch的vis_output取出，分别计算一小块相似度矩阵,最终拼接在一起
            b1b2_logits, *_tmp = model.get_similarity_logits(sequence_output, visual_output, input_mask, video_mask,
                                                                     loose_type=model.loose_type, eval=eval)
           
            b1b2_logits = b1b2_logits.cpu().detach().numpy()
            each_row.append(b1b2_logits)
        each_row = np.concatenate(tuple(each_row), axis=-1)  #####行拼接
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):    #验证重点，主要内容
    ...
    with torch.no_grad():   #不追踪梯度操作，可以显著减少显存占用
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            # video torch.Size([16, 1, 12, 1, 3, 224, 224])
            # video mask torch.Size([16, 1, 12])
            # input_ids ([16, 1, 32])
            input_ids, input_mask, segment_ids, video, video_mask = batch
            ...
            if multi_sentence_:
                ...
                # video 每个视频 ([16, 1, 12, 1, 3, 224, 224])
        ...
        if n_gpu > 1:
            ...
            sim_matrix = np.concatenate(tuple(sim_matrix), axis=0)  ########列拼接

            sim_matrix = torch.from_numpy(sim_matrix)
            sim_matrix = nn.functional.softmax(sim_matrix, dim=1) * nn.functional.softmax(sim_matrix, dim=0)
            sim_matrix = sim_matrix.numpy()

            mylist = [841]
            for i in mylist:
                index = np.argsort(-sim_matrix[i])
                print(index[:5])
            # num = 1000
            # mylist = []
            # sort = np.argsort(sim_matrix)
            # for i in range(num):
            #     for j in range(num):
            #         if sort[i][j] == i:
            #             #print((i,j))
            #             mylist.append((i,999-j))
            ...
            # import json
            # a = {'a': mylist}
            # with open("a+b.json",'w',encoding='utf-8') as f:
            #     json.dump(a,f,ensure_ascii=False)
            #print(mylist)
            #print(np.diagonal(np.argsort(sim_matrix)))
        else:
