...
class RawVideoExtractorCV2():
    ...
    def video_to_tensor(self, video_file, preprocess, sample_idx):
    ...
    def get_video_data(self, video_path, sample_idx):
        image_input = self.video_to_tensor(video_path, self.transform, sample_idx=sample_idx)
        return image_input
...
'''testExtractor = RawVideoExtractorCV2(framerate=2)
video_path = '/data1/ckli/MSRVTT/MSRVTT_Videos/video13.mp4'
image_input = testExtractor.get_video_data(video_path,start_time=0,end_time=10)
frame = image_input['video'][1].numpy()
...
class PicNet(nn.Module):
    def __init__(self,channel=3):
        ...
        self.pool = nn.AvgPool2d(kernel_size=(6,6),stride=(6,6),padding=2)
    def forward(self,x):
        return self.pool(x)
         '''